...
async def cmd_start(message: Message, bot: Bot, session: AsyncSession):
    ...
    string = (f"👋 Я — Master Manifest и готов с тобой поделиться всей той информацией, которой обладаю"
              f"\n\nМоя задача — объяснять сложное простым языком, чтобы помочь тебе раскрыть силу мысли, изменить "
              f"внутренние установки и научить, как направлять энергию на исполнение желаний "
